...
app.get('/forms',   (_req, res) =>  { // putting _ in front means we do not want to use it here, so stop never-used warning 
     // res.send('Hello From Express Server! - Watch Mode :) ');
     res.json(questions);
});
 
//any name can be used besides serve-client or /main.js as well 
app.get('/main.js',   (_req, res) =>  { // putting _ in front means we do not want to use it here, so stop never-used warning 
